...
4. Write ``order.json`` with the feature order expected by ``sweep_models.py``.
5. Build a reduced training directory with only the selected columns.
6. Optionally delegate training to ``train_mps_nsl_kdd.main``.
...
-------------
    ...
    # Then reuse the generated ranking in the sweep driver:
    python sweep_models.py /path/to/nsl_kdd --feature-order /path/to/nsl_kdd/order.json
...
def feature_order_from_trajectory(trajectory: Sequence[Mapping]) -> List[int]:
    """Return sites ordered from most to least important for ``sweep_models.py``.

    Greedy backward elimination removes the least useful remaining site first.
    Therefore the sweep ranking is the final surviving site(s), followed by the
    removal sequence reversed. When ``min_features > 1``, the final survivors are
    tied by the elimination run and keep their current deterministic site order.
    """
    if not trajectory:
        raise ValueError("empty trajectory")

    removed_least_to_most = [
        int(row["removed_next"])
        for row in trajectory
        if row.get("removed_next") is not None
    ]
    final_kept = [int(site) for site in trajectory[-1].get("kept_sites", [])]
    order = final_kept + list(reversed(removed_least_to_most))

    if len(order) != len(set(order)):
        raise ValueError(f"feature order contains duplicates: {order}")

    first_kept = trajectory[0].get("kept_sites")
    if first_kept is not None:
        expected = {int(site) for site in first_kept}
        got = set(order)
        if got != expected:
            missing = sorted(expected - got)
            extra = sorted(got - expected)
            raise ValueError(f"feature order mismatch; missing={missing}, extra={extra}")

    return order


def save_feature_order(
    trajectory: Sequence[Mapping],
    feature_names: Sequence[str],
    ...
    *,
    ...
    """Save an ``order.json`` compatible with ``sweep_models.py --feature-order``.

    ``sweep_models.py`` accepts a JSON object containing an ``order`` key. The
    list is ranked from most to least important, so ``order[:k]`` is the
    selected subset for model size ``k``.
    """
    output_json = Path(output_json)
    output_json.parent.mkdir(parents=True, exist_ok=True)

    order = feature_order_from_trajectory(trajectory)
    removed_least_to_most = [
        int(row["removed_next"])
        for row in trajectory
        if row.get("removed_next") is not None
    ...
    payload: Dict = {
        "order": order,
        "feature_names": [feature_names[site] for site in order],
        "ranking_direction": "most_to_least_important",
        "method": "selection_only_backward_elimination",
        "objective": objective if isinstance(objective, str) else "custom",
        "removed_order_least_to_most": removed_least_to_most,
        "removed_feature_names_least_to_most": [feature_names[site] for site in removed_least_to_most],
        "min_features_reached": int(trajectory[-1]["n_features"]),
    }
    if selected_row is not None:
        payload["selected_k"] = int(selected_row["n_features"])
        payload["selected_sites"] = [int(site) for site in selected_row["kept_sites"]]
        payload["selected_features"] = list(selected_row["kept_features"])
        payload["selected_select_objective"] = float(selected_row["select_objective"])
        payload["selected_select_auc_roc"] = float(selected_row["select_auc_roc"])
        payload["selected_select_auc_pr"] = float(selected_row["select_auc_pr"])
    if trajectory_path is not None:
        payload["selection_trajectory"] = str(trajectory_path)
...
def build_reduced_training_dir(
    ...
    *,
    ...
    if feature_order_path is not None:
        provenance["feature_order"] = str(feature_order_path)
    if selected_row is not None:
...
def run(
    ...
    *,
    ...
    order_json: Optional[Union[str, Path]] = None,
    ...
    order_json_path = save_feature_order(
        trajectory,
        feature_names,
        order_json,
        objective=objective,
    ...
    logger.info("wrote feature order for sweep: %s", order_json_path)
    ...
    out_dir = build_reduced_training_dir(
        ...
        selected_row=selected_row,
        trajectory_path=json_path,
        feature_order_path=order_json_path,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-features", type=int, default=2, help="smallest subset considered during BE")
    ...
    train.add_argument("--order-json", type=Path, default=None,
                       help="where to save the feature ranking for sweep_models.py; defaults to <data_dir>/order.json")
